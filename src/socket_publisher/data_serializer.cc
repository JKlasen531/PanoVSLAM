...
data_serializer::data_serializer(const std::shared_ptr<openvslam::publish::frame_publisher>& frame_publisher,
                                 ...
                                 const std::string& img_output_path,
                                 bool publish_points)
    : frame_publisher_(frame_publisher), map_publisher_(map_publisher), image_publisher_(image_publisher), img_output_path_(img_output_path),
      publish_points_(publish_points), latest_image_id_(0),
...
void data_serializer::test_image_publisher() {
    std::vector<openvslam::data::keyframe*> keyframes;
    image_publisher_->test_image_db(keyframes);
}
...
std::string data_serializer::serialize_map_diff() {
    ...
    image_publisher_->get_images(images);

    return serialize_as_protobuf(keyframes, all_landmarks, local_landmarks, current_camera_pose, images);
}
...
std::string data_serializer::serialize_as_protobuf(const std::vector<openvslam::data::keyframe*>& keyfrms,
    ...
    for (const auto keyfrm : keyfrms) {
        ...
        const auto srcfrmid = keyfrm->src_frm_id_;
        ...
        keyfrm_obj->set_srcfrmid(keyfrm->src_frm_id_);
    ...
    *point_hash_map_ = next_point_hash_map;
    ...
    //images
    unsigned int next_latest_image_id_ = 0;
    for (const auto image : images) {
        auto path = image;
        path.erase(0, (img_output_path_.length()+5));
        path.erase(path.length()-4,4);
        int id = stoi(path);
        std::filesystem::path p = std::filesystem::absolute(image);
        std::string imgPathAbs = p.string();
        if(id >= latest_image_id_) {
            auto img = map.add_images();
            img->set_path(imgPathAbs);
            if(id > next_latest_image_id_) {
                next_latest_image_id_ = id;
            }
        }
    }
    latest_image_id_ = next_latest_image_id_;

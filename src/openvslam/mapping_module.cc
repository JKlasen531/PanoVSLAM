...
mapping_module::mapping_module(const YAML::Node& yaml_node, data::map_database* map_db, data::image_database* img_db,
    const std::string& image_output_path)
    : local_map_cleaner_(new module::local_map_cleaner()), map_db_(map_db), img_db_(img_db), image_output_path_(image_output_path),
...
void mapping_module::store_new_keyframe() {
    ...
    save_keyfrm_img(rgb, s_src_frm_id, image_output_path_);
}

// add to h file
void mapping_module::save_keyfrm_img(cv::Mat& keyfrm_rgb, std::string &id, std::string &path_to_save) {
    std::string path_rgb_full = (path_to_save + "/rgb_" + id + ".png");
    cv::imwrite(path_rgb_full, keyfrm_rgb);
    img_db_->add_image(path_rgb_full);
}

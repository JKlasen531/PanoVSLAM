...
system::system(const std::shared_ptr<config>& cfg, const std::string& vocab_file_path, const std::string& project_folder_path)
    : cfg_(cfg), camera_(cfg->camera_), project_folder_path_(project_folder_path) {
    ...
    img_db_ = new data::image_database();
    ...
    image_publisher_ = std::shared_ptr<publish::image_publisher>(new publish::image_publisher(img_db_, map_db_));
    //std::cout << "testing publisher" << std::endl;
    //std::vector<std::string*> paths;
    //image_publisher_->get_images(paths);

    // tracking module
    tracker_ = new tracking_module(cfg_, this, map_db_, bow_vocab_, bow_db_);
    // mapping module
    mapper_ = new mapping_module(cfg_->yaml_node_["Mapping"], map_db_, img_db_, project_folder_path_);
...
const std::shared_ptr<publish::image_publisher> system::get_image_publisher() const {

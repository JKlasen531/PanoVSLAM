...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    // build a SLAM system
    openvslam::system SLAM(cfg, vocab_file_path, img_output_path);
    ...
    socket_publisher::publisher publisher(
        openvslam::util::yaml_optional_ref(cfg->yaml_node_, "SocketPublisher"), &SLAM, SLAM.get_frame_publisher(), SLAM.get_map_publisher(), SLAM.get_image_publisher(), img_output_path);
#endif
    ...
    std::thread thread([&]() {
        while (is_not_end) {
            ...
            bool success = imwrite("./frames/" + std::to_string(timestamp) + ".jpg", frame);
    ...
    viewer.run();
#elif USE_SOCKET_PUBLISHER
    publisher.init(std::to_string(cam_num), img_output_path ,"cameraSlam", video.get(cv::CAP_PROP_FPS), 1);
    publisher.run();
#endif
...
void stereo_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    viewer.run();
#elif USE_SOCKET_PUBLISHER
    publisher.init(std::to_string(cam_num), img_output_path ,"cameraSlam", videos[0].get(cv::CAP_PROP_FPS), 1);
    publisher.run();
#endif
...
int main(int argc, char* argv[]) {
    ...
    auto img_output_path = op.add<popl::Value<std::string>>("", "img-output", "location where the keyframe images will be saved", "");
    try {
        op.parse(argc, argv);
    }
    ...
    if (!vocab_file_path->is_set() || !cam_num->is_set()
        || !config_file_path->is_set() || !img_output_path->is_set()) {
    ...
    if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
        mono_tracking(cfg, vocab_file_path->value(), cam_num->value(), mask_img_path->value(),
                      scale->value(), map_db_path->value(), img_output_path->value());
    }
